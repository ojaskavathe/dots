...
- debugging: `herdr agent explain` dumps matched rule + per-rule evidence
  as JSON. worth copying as `winch agent explain`.
...
winch is a product — a tmux sidecar for anyone, not just this dotfiles
...
hooks are strictly opt-in (`winch hooks install` owning its own entries,
...
(~/.config/winch/agents/*.toml) wins, so UI-copy breakage is a user patch,
not a release.
