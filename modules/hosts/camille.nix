# Host: camille (aarch64-darwin, single-user `ojas`).
# primaryUser is injected as a module arg so homebrew (and any other module that
# needs it) can read it without specialArgs. allowUnfree comes from the shared
# nixpkgs module, so it's no longer set here.
{ ... }:
{
  flake.modules.darwin.camille =
    {
      config,
      lib,
      pkgs,
